...
describe("Add movie webpage", function () {
  ...
  it("fill and submit form", function () {
    ...
    cy.get('input[name="cast"]').type(
      "Director, Dean SWD, Dean GAD, Professors"
    );
    cy.get('textarea[name="desc"]').type(
      "A history of BPHC over the years visualised"
    );
    cy.screenshot();
  ...
  it("error on entering movie before 1888", function () {
    ...
    cy.get('textarea[name="desc"]').type(
      "Fake news being used to modify history"
    );
    cy.screenshot();
    var infoboxDisplayed = false;
    cy.on("window:alert", (str) => {
      expect(str).to.equal(
        "Year must be a number greater than 1888. The first movie in the world was released in 1888"
      );
      infoboxDisplayed = true;
    });
    cy.get('input[type="submit"]').click();
  ...
  it("error on entering movie after 2020", function () {
    ...
    cy.get('textarea[name="desc"]').type(
      "Predicting the future using mathematics was Hari Seldon's idea, not yours"
    );
    cy.screenshot();
    var infoboxDisplayed = false;
    cy.on("window:alert", (str) => {
      expect(str).to.equal(
        "You are allowed to enter only already released movies."
      );
      infoboxDisplayed = true;
    });
    cy.get('input[type="submit"]').click();
